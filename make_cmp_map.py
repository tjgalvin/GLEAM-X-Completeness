...
def read_catalogue(input_catalogue):
    ...
    ra=table.field("ra"); dec=table.field("dec")
    return ra, dec
...
usage="Usage: %prog [options] <injected_sources> <detected_sources> \n"\
...
parser.add_option('--flux',type="string", dest="flux", help="Fluxes at which to calculate completeness. "\
"Enter flux_min, flux_max, flux_interval, in mJy (e.g. 5,10,1). This parameter must be given. No default.")
parser.add_option('--template_map',type="string", dest="template_map", help="Template FITS map for "\
"generating completeness maps. "\
"This can be set to the PSF map. If not given, no completeness maps are generated. No default.")
parser.add_option('--region',type="string", dest="region", default="0,360,-90,90", help="Sky coverage of "\
...
parser.add_option('--rad',type="float", dest="rad", default=6.0, help="Radius of circle centred on each "\
"pixel in which to calculate the completeness, in deg [default=%default]")
parser.add_option('--output_root',type="string", dest="output_root", default='completeness', help="Output "\
"root filename [default=%default]")
(options, args) = parser.parse_args()
injected_sources=args[0]; detected_sources=args[1]
if not options.flux:
...
ra_inj=np.array(ra_inj); dec_inj=np.array(dec_inj)
ninj=len(ra_inj)
...
for i in range(0,sdim):
    ...
    ra_det.append(ra); dec_det.append(dec)
ra_det=np.array(ra_det); dec_det=np.array(dec_det)
...
# Calculate completeness map at each flux level, blanking out pixels outside the specified region
    ...
    shape=data.shape
    ydim=shape[1]; xdim=shape[2]
    shape=(sdim, ydim, xdim)
    cmp_cube=np.empty(shape); cmp_cube[:]=np.nan
    step=1.0; f=step; n=0
        ...
        for k in range(0,xdim):
            ...
            if dec >= dec_min and dec <= dec_max:
                ...
                (ra_max < ra_min and (ra >= ra_min or ra <= ra_max)):
                    ...
                    for i in range(0,sdim):
                        ndet_rad=count_sources(ra,dec,ra_det[i],dec_det[i],options.rad)
